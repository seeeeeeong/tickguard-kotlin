...
| 매일 자동 주문 켜기 → 켜기 | Runs the dip sleeve, configured `DRY_RUN`, live every weekday without a press. Announced in Discord | Until switched off on the page, or by the stop button; kept across restarts in `data/placements/daily-live` |
...
Only the daily switch survives a restart, kept in `data/placements/daily-live`, shown at the top of the page
